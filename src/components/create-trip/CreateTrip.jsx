...
export default function CreateTrip() {
  ...
  const [formData, setFormData] = useState({ destination: "", days: "", budget: "", people: "" });
  ...
  const onGenerateTrip = async () => {
    ...
    if (!days || Number(days) <= 0 || Number(days) > 10) {
      toast.error("Please enter a valid number of days (1-10)!", { autoClose: 2000 });
      return;
    }
    if (!budget) {
    ...
    if (!people) {
      toast.error("Please select who you are traveling with!", { autoClose: 2000 });
      return;
    }
  ...
  return (
    <div className="sm:px-10 md:px-32 lg:px-56 xl:px-10 px-5 mt-10">
      ...
      <p className="mt-3 text-gray-600 text-xl">
        Just provide some basic information, and our AI trip planner will craft a personalized itinerary for you.
      </p>
      ...
      <div className="p-5">
        <h2 className="text-xl mt-5">How many days are you planning your trip?</h2>
        <Input type="number" placeholder="Ex-3" onChange={(e) => handleInputChange("days", e.target.value)} />
      </div>
          ...
          {SelectBudgetOptions.map((item, index) => (
            <div
              ...
              className={`p-5 border rounded-lg cursor-pointer hover:shadow-lg ${
                formData.budget === item.title ? "shadow-gray-500 border-black" : ""
              }`}
          ...
          {SelectTravelesList.map((item, index) => (
            <div
              ...
              className={`border rounded-lg cursor-pointer hover:shadow-lg p-5 ${
                formData.people === item.people ? "shadow-gray-500 border-black" : ""
              }`}
      ...
      <Dialog open={openDialogBox}>
        <DialogContent>
          <DialogHeader>
            <DialogDescription>
              <h2 className="font-bold text-black text-lg mt-7">Sign in with Google</h2>
